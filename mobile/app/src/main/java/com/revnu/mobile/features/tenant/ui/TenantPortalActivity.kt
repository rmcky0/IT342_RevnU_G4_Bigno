            ...
            try {
                ...
                if (!response.isSuccessful) {
                    if (response.code() == 401 || response.code() == 403) {
                        Toast.makeText(this@TenantPortalActivity, "Session expired. Please log in again.", Toast.LENGTH_SHORT).show()
                        sessionManager.clearSession()
                        startActivity(Intent(this@TenantPortalActivity, LoginActivity::class.java))
                        finish()
                    }
                }
            } catch (e: Exception) {
